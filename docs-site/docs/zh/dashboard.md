...
- **团队 / Roles / Bot Defaults**：团队面板做[跨部署协作](/roles)（邀请别人的部署进团队、跨部署拉群）；Roles 管理各 bot 按群人设；Bot Defaults（Bot 配置）配默认行为（新群 oncall、卡片签名、**默认角色**等）。其中「机器人 → 高级 → 会话 Owner 定时提醒」可按周一至周日分别配置多个分钟级提醒范围，显示当前全局定时任务时区；默认工作日 `10:30–21:30`，窗口外继续累计但不发送。
