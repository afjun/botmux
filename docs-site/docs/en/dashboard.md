...
- **Team / Roles / Bot Defaults**: the Team panel handles [cross-deployment collaboration](/en/roles) (invite someone else's deployment into your team, create cross-deployment groups); Roles manages each bot's per-group persona; Bot Defaults (Bot configuration) sets default behaviors (new-group on-call, card signature, **default role**, etc.). Under **Bot → Advanced → Session Owner reminders**, each weekday can have multiple minute-level reminder windows and displays the current global schedule timezone. The default is `10:30–21:30` on weekdays; waiting continues outside the window but delivery is suppressed.
